...
static void initialize_spi_clocks(spi_dev_t *spiHw) {
   ...
   //			In master mode, this must be equal to SPI_CLKCNT_N.
   {
      ...
      const double apbDivider = (apbClockSpeedInHz / preDivider / 20000000);
...
void mcp4822_initialize(mcp4822_t *dac) {
   ...
   gpio_set_direction(dac->cs_pin, GPIO_MODE_INPUT_OUTPUT);
...
//   gpio_matrix_in(ldac_pin, HSPICS0_IN_IDX, true);
   ...
   dac->spiHw->mosi_dlen.usr_mosi_dbitlen = 15;
   dac->spiHw->user.usr_mosi = 1;
   initialize_spi_clocks(dac->spiHw);
   gpio = &GPIO;
   ESP_LOGI(TAG, "DAC Initialized: %x", gpio->enable);
}

static uint16_t channel_a;
static uint16_t channel_b;

void IRAM_ATTR mcp4822_set(mcp4822_t *dac, int16_t out1, int16_t out2) {
   channel_a = out1 & 0xFFF;
